#include <iostream>
#include <string>
using namespace std;

// plantilla

template<class T>
class LSLSE;

template<class T>
class node{
private:
    T data; // Guarda la información (por ejemplo un objeto SocioClub)
    node<T>* sig;     // Apunta al siguiente nodo

public:
    node():sig(nullptr){};   // Constructor: siguiente nulo
    friend class LSLSE<T>;   // Permite que LSLSE acceda a sus atributos privados
};

template<class T>
class LSLSE{
private:
    node<T>* lista; //puntero al primer nodo
public:
    LSLSE():lista(nullptr){}; // Constructor: lista vacía (puntero nulo)
    bool vacia()const;
    node<T>* ultimo()const;
    node<T>* primero()const;
    node<T>* anterior(node<T>* pos)const;
    void insertar(node<T>* pos, T elem);
    bool eliminar(node<T>* pos);
    void imprimir()const;

    // --- MÉTODOS PÚBLICOS NUEVOS PARA ACCESO CONTROLADO ---
    T& getData(node<T>* pos) const { return pos->data; }
    node<T>* getNext(node<T>* pos) const { return pos->sig; }

    // --- FUNCIONES PERSONALIZADAS ---
    node<T>* buscarPorNumero(int num);
    node<T>* buscarPorNombreYDom(string nombre, string dom);
    int contarSocios() const;
};


//Recorre toda la lista e imprime el contenido de cada nodo.
template<class T> //IMPRIME LOS SOCIOS
void LSLSE<T>::imprimir()const{
    node<T>* aux=lista;
    while(aux!=nullptr){
        cout<<aux->data<<"-> ";
        aux=aux->sig;
    }
}

template<class T> //ELIMINAR
bool LSLSE<T>::eliminar(node<T>* pos){
    if(vacia() || pos==nullptr){
        return false;
    }
    if(pos==lista){
        lista=lista->sig;
    }
    else{
        anterior(pos)->sig=pos->sig;
    }
    delete pos;
    return true;
}

template<class T>//INSERTAR
void LSLSE<T>::insertar(node<T>* pos, T elem){
    node<T>* aux= new node<T>;
    aux->data=elem;
    if(pos==nullptr){
        aux->sig=lista;
        lista=aux;
    }
    else{
        aux->sig=pos->sig;
        pos->sig=aux;
    }
}



template<class T>//Busca el nodo que apunta al nodo pos
node<T>* LSLSE<T>::anterior(node<T>* pos)const{
    if(vacia() || pos==nullptr){
        return nullptr;
    }
    node<T>* aux=lista;
    while(aux!=nullptr && aux->sig!=pos){
        aux=aux->sig;
    }
    return aux;
}

template<class T>
node<T>* LSLSE<T>::primero()const{
    if(vacia()){
        return nullptr;
    }
    return lista;
}

template<class T>
node<T>* LSLSE<T>::ultimo()const{
    if(vacia()){
        return nullptr;
    }
    node<T>* aux=lista;
    while(aux->sig!=nullptr){
        aux=aux->sig;
    }
    return aux;
}

template<class T>
bool LSLSE<T>::vacia()const{
    return lista==nullptr;
}


//CLASE SOCIOCLUB
class SocioClub {
private:
    int numeroSocio;
    string nombreSocio;
    string domicilio;
    int anioIngreso;

public:
    // Constructores
    SocioClub(): numeroSocio(0), nombreSocio(""), domicilio(""), anioIngreso(0) {}
    SocioClub(int num, string nom, string dom, int anio)
        : numeroSocio(num), nombreSocio(nom), domicilio(dom), anioIngreso(anio) {}

    // Métodos de acceso
    int getNumeroSocio() const { return numeroSocio; }
    string getNombre() const { return nombreSocio; }
    string getDomicilio() const { return domicilio; }
    int getAnioIngreso() const { return anioIngreso; }

    // Sobrecarga de operadores
    bool operator==(const SocioClub& otro) const {
        return numeroSocio == otro.numeroSocio;// Igualdad por número de socio
    }

    bool operator<(const SocioClub& otro) const {
        return numeroSocio < otro.numeroSocio; // Orden natural: por número
    }

    friend ostream& operator<<(ostream& os, const SocioClub& s) {
        os << "\nNumero de Socio: " << s.numeroSocio
           << "\nNombre: " << s.nombreSocio
           << "\nDomicilio: " << s.domicilio
           << "\nAño de Ingreso: " << s.anioIngreso << "\n";
        return os;
    }
};

//  Metodos para  a SocioClub

template<class T> //BUSCAR SOCIO POR NUMERO
node<T>* LSLSE<T>::buscarPorNumero(int num){
    node<T>* aux=lista;
    while(aux!=nullptr){
        if(aux->data.getNumeroSocio()==num)
            return aux;
        aux=aux->sig;
    }
    return nullptr;
}

template<class T>//BUSCAR SOCIO POR NOMBRE
node<T>* LSLSE<T>::buscarPorNombreYDom(string nombre, string dom){
    node<T>* aux=lista;
    while(aux!=nullptr){
        if(aux->data.getNombre()==nombre && aux->data.getDomicilio()==dom)
            return aux;
        aux=aux->sig;
    }
    return nullptr;
}

template<class T> //CUENTA LOS SOCIOS TOTALES
int LSLSE<T>::contarSocios() const{
    int c=0;
    node<T>* aux=lista;
    while(aux!=nullptr){
        c++;
        aux=aux->sig;
    }
    return c;
}

//manin

int main(){
    LSLSE<SocioClub> lista;
    int opcion;

    do{
        cout << "\SOCIOS DE CLUB =-) \n";
        cout << "1. Registrar nuevo socio\n";
        cout << "2. Dar de baja un socio\n";
        cout << "3. Generar reporte de socios\n";
        cout << "4. Buscar socio (nombre y domicilio)\n";
        cout << "5. Total de socios registrados\n";
        cout << "0. Salir\n";
        cout << "Seleccione una opcion: ";
        cin >> opcion;

        if(opcion==1){
            int num, anio;
            string nombre, dom;

            cout << "Numero de socio: ";
            cin >> num;
            cin.ignore();
            cout << "Nombre: ";
            getline(cin, nombre);
            cout << "Domicilio: ";
            getline(cin, dom);
            cout << "Anio de ingreso: ";
            cin >> anio;

            SocioClub s(num, nombre, dom, anio);

            if(lista.buscarPorNumero(num)){
                cout << "Ya existe un socio con ese numero.\n";
            } else {
                node<SocioClub>* aux = lista.primero();
                node<SocioClub>* posAnterior = nullptr;

                // Acceso seguro a data y sig
                while(aux != nullptr && lista.getData(aux) < s){
                    posAnterior = aux;
                    aux = lista.getNext(aux);
                }
                lista.insertar(posAnterior, s);
                cout << "Socio registrado correctamente.\n";
            }

        } else if(opcion==2){
            int num;
            cout << "Ingrese numero de socio a eliminar: ";
            cin >> num;
            node<SocioClub>* pos = lista.buscarPorNumero(num);
            if(lista.eliminar(pos))
                cout << "Socio eliminado.\n";
            else
                cout << "No se encontro  socio.\n";

        } else if(opcion==3){
            lista.imprimir();

        } else if(opcion==4){
            string nombre, dom;
            cin.ignore();
            cout << "Nombre: ";
            getline(cin, nombre);
            cout << "Domicilio: ";
            getline(cin, dom);
            node<SocioClub>* pos = lista.buscarPorNombreYDom(nombre, dom);
            if(pos)
                cout << lista.getData(pos);
            else
                cout << "No se encontro socio.\n";

        } else if(opcion==5){
            cout << "Total de socios registrados: " << lista.contarSocios() << "\n";
        }

    }while(opcion!=0);

    cout << "\nPrograma finalizado.\n";
    return 0;
}
